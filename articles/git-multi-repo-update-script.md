...
# (set -e は使わない。失敗したリポジトリを数えて続行し、最後に集計するため)
...
※ macOS標準のBash 3.2で動作確認済み。絵文字表示にはUTF-8対応のターミナルが必要です。

## 注意点

- stashの競合が発生した場合は手動で解決が必要です
- リモートへの認証（SSH鍵など）が設定済みである必要があります
- mainブランチが無いリポジトリ（master運用など）は「切り替え失敗」として数え、次のリポジトリへ進みます。その際、直前にstashした変更は自動復元されないため、`git stash list` で確認してください
- `set -e` を足すと、mainの無いリポジトリに当たった時点でスクリプト全体が終了し、残りのリポジトリが未処理のままになります（サマリーも出ません）

## まとめ

配下のリポジトリを自動検出し、mainへの切り替えとpullを一括実行して結果を集計します。失敗したリポジトリはスキップして続行するので、週明けにまとめて最新化する用途に向いています。
