...
:::message
多くのプロジェクトでは Snackbar と Toast を区別せず「Toast」と呼ぶことが一般的である。重要なのは用語の統一よりも、「ユーザーの操作を阻害しない一時通知」と「注意を引く必要がある永続的な通知」を明確に区別することだ。本章では一時通知全般を「Snackbar / Toast」として扱う。
:::
...
確認ダイアログで操作を止めるのではなく、操作を即座に実行してから取り消しの選択肢を提供するアプローチである。破壊的操作の UX を劇的に改善する。
