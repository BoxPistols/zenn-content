...
管理画面、ダッシュボード、データ一覧など、テーブルは実務で頻繁に登場する。しかし「div で組む」「ellipsis で切り詰める」「入れ子テーブルで階層を表現する」といった安易な実装は、アクセシビリティの崩壊やレスポンシブ不可の原因になる。ここでテーブル設計の全体像を把握し、どのようなデータにも対応できる力を身につけてほしい。
...
- カラム数が 5 以上で、各カラムに最低限必要な幅がある場合
- テーブルの最小幅が 768px 以上になる場合（モバイルに収まらない）
