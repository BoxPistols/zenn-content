...
関数本体はクイックスタートStep 3で追加した `labels-sync` と同一です（Zshは `~/.zshrc`、Bashは `~/.bashrc`、中身は同一）。`gh repo view` でカレントディレクトリのリポジトリを特定し、第1引数がファイル名なら設定ファイルとして、`--` で始まればオプションとしてそのまま `labels-config sync` に渡します。
