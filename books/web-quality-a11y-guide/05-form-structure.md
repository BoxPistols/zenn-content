---
title: "Formの構造とアクセシビリティ"
---

フォームはWebで最もインタラクティブな要素であり、アクセシビリティ上の問題が最も起きやすい箇所である。HTMLの構造から、ラベルの紐付け、バリデーション、エラーの伝達、送信時のUX、Reactでの実装までを扱う。
...
`select` や `input type="date"` は見た目を揃えたくなるが、作り替えるほどキーボード操作と支援技術への対応を自前で持つことになる。必要になる対応は「よくあるフォームの課題と問題点」で扱う。
...
- バリデーション: JSメインにネイティブ属性をセマンティクスとして併用。判定はonBlurと送信時に行う
- エラー: `aria-invalid` と `aria-describedby` で伝え、サマリーにフォーカスを移す
- 送信: `aria-busy` で処理中を示し、結果にフォーカスを移す
- 自動入力: `autocomplete` を設定する。入力負担の軽減は認知障害や運動障害のあるユーザーにも効く
